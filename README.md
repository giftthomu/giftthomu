<h1 align="center">Hi 👋, I'm Gift Thomu</h1>
<h3 align="center">Software Developer | Future Solutions Architect | Building Scalable Digital Solutions</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=giftthomu&label=Profile%20Views&color=blueviolet&style=for-the-badge" alt="giftthomu" />
</p>

---

## 🚀 About Me

- 💼 Software Developer at **Malawi Revenue Authority (MRA)**
- 🏛️ Contributing to Malawi's **Electronic Invoicing System (EIS)**
- 📱 Building modern Android applications with **Kotlin & Jetpack Compose**
- 🌐 Developing enterprise systems using **ASP.NET Core & .NET**
- 🏗️ Designing scalable architectures and business solutions
- 🎯 Working towards becoming a **Solutions Architect**
- 🤝 Open to collaborating on innovative software projects
- 💬 Ask me about **Kotlin, Jetpack Compose, .NET, ASP.NET Core, SQL Server, PostgreSQL, API Design, System Architecture**
- 📫 Reach me at: **giftthomu@gmail.com**

---

## 🌐 Connect With Me

<p align="left">
<a href="https://linkedin.com/in/giftthomu" target="_blank">
<img src="https://skillicons.dev/icons?i=linkedin" height="40" />
</a>
<a href="https://github.com/giftthomu" target="_blank">
<img src="https://skillicons.dev/icons?i=github" height="40" />
</a>
</p>

---

## 🛠️ Tech Stack

### Backend Development
<p>
<img src="https://skillicons.dev/icons?i=dotnet,cs,nodejs,spring" />
</p>

### Mobile Development
<p>
<img src="https://skillicons.dev/icons?i=kotlin,androidstudio,flutter" />
</p>

### Databases
<p>
<img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,sqlite" />
</p>

### Cloud, DevOps & Tools
<p>
<img src="https://skillicons.dev/icons?i=azure,docker,git,github,postman,firebase" />
</p>

### Frontend & Design
<p>
<img src="https://skillicons.dev/icons?i=html,css,js,react,figma" />
</p>

---

## 🏗️ Current Focus

```text
✓ Electronic Invoicing Systems (EIS)
✓ Enterprise Application Development
✓ Kotlin + Jetpack Compose
✓ ASP.NET Core APIs
✓ PostgreSQL & SQL Server
✓ System Architecture
✓ Digital Payments & POS Systems
✓ Distributed Systems & Microservices
